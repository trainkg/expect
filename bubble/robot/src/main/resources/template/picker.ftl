...
<template>
  <a-collapse v-model="activeKey" expand-icon-position="left">
    ...
    <a-collapse-panel key="search" header="查询条件">
      ...
      <@mfield formConfig=formConfig fields=config.fields/>
...
const columns = ${tableWapper.columnJSON}
...
export default {
  ...
  data() {
    return {
      ...
      form: ${tableWapper.columnFeildJSON}
    }
  },
  computed: {
    rowSelection() {
  ...
  methods: {
    ...
    fetch(params = {}) {
      ...
      pageSearchData('${beanName}', pageParam, this.form).then(rs => {
